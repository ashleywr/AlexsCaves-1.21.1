...
public class ConversionCrucibleBlockRenderer<T extends ConversionCrucibleBlockEntity> implements BlockEntityRenderer<T> {
    ...
    public void render(T crucible, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn) {
        ...
        MODEL.renderToBuffer(poseStack, bufferIn.getBuffer(RenderType.entityCutoutNoCull(TEXTURE)), combinedLightIn, combinedOverlayIn, white);
        ...
        if(crucible.getFilledLevel() > 0){
            ...
            int colorArgb = (0xFF << 24) | ((int)(r * 255) << 16) | ((int)(g * 255) << 8) | (int)(b * 255);
            MODEL.renderToBuffer(poseStack, bufferIn.getBuffer(ACRenderTypes.getVoidBeingCloud(TEXTURE_FLUID)), combinedLightIn, combinedOverlayIn, colorArgb);
        }
        MODEL.hideBeam(false);
        MODEL.hideSauce(true);
        float alpha = Math.max(splashProgress, bob1 * conversionProgress);
        int colorArgbAlpha = ((int)(alpha * 255) << 24) | ((int)(r * 255) << 16) | ((int)(g * 255) << 8) | (int)(b * 255);
        MODEL.renderToBuffer(poseStack, bufferIn.getBuffer(ACRenderTypes.getVoidBeingCloud(TEXTURE_OVERLAY)), combinedLightIn, combinedOverlayIn, colorArgbAlpha);
        poseStack.popPose();
        ...
        if(showItemProgress > 0.0F) {
            ...
            shineOriginVertex(lightConsumer, matrix4f2, posestack$pose1, 0, 0, r, g, b, 12 * showItemProgress);
            shineLeftCornerVertex(lightConsumer, matrix4f2, posestack$pose1, lightLength, lightWidth, 0, 0, r, g, b, 0);
            shineRightCornerVertex(lightConsumer, matrix4f2, posestack$pose1, lightLength, lightWidth, 0, 0, r, g, b, 0);
            shineLeftCornerVertex(lightConsumer, matrix4f2, posestack$pose1, lightLength, lightWidth, 0, 0, r, g, b, 0);
            poseStack.popPose();
        }
    }
    ...
    private static void shineOriginVertex(VertexConsumer vertexConsumer, Matrix4f matrix4f, PoseStack.Pose pose, float xOffset, float yOffset, float r, float g, float b, float a) {
        vertexConsumer.addVertex(matrix4f, 0.0F, 0.0F, 0.0F).setColor((int)(r * 255), (int)(g * 255), (int)(b * 255), (int)(a * 255)).setUv(xOffset + 0.5F, yOffset).setOverlay(OverlayTexture.NO_OVERLAY).setUv2(240 & 0xFFFF, 240 >> 16).setNormal(pose, 0.0F, 1.0F, 0.0F);
    }

    private static void shineLeftCornerVertex(VertexConsumer vertexConsumer, Matrix4f matrix4f, PoseStack.Pose pose, float length, float width, float xOffset, float yOffset, float r, float g, float b, float a) {
        vertexConsumer.addVertex(matrix4f, -ACMath.HALF_SQRT_3 * width, length, 0).setColor((int)(r * 255), (int)(g * 255), (int)(b * 255), (int)(a * 255)).setUv(xOffset, yOffset + 1).setOverlay(OverlayTexture.NO_OVERLAY).setUv2(240 & 0xFFFF, 240 >> 16).setNormal(pose, 0.0F, -1.0F, 0.0F);
    }

    private static void shineRightCornerVertex(VertexConsumer vertexConsumer, Matrix4f matrix4f, PoseStack.Pose pose, float length, float width, float xOffset, float yOffset, float r, float g, float b, float a) {
        vertexConsumer.addVertex(matrix4f, ACMath.HALF_SQRT_3 * width, length, 0).setColor((int)(r * 255), (int)(g * 255), (int)(b * 255), (int)(a * 255)).setUv(xOffset + 1, yOffset + 1).setOverlay(OverlayTexture.NO_OVERLAY).setUv2(240 & 0xFFFF, 240 >> 16).setNormal(pose, 0.0F, -1.0F, 0.0F);
    }
