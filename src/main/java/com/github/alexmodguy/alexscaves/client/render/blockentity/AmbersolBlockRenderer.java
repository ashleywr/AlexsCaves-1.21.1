...
public class AmbersolBlockRenderer<T extends AmbersolBlockEntity> implements BlockEntityRenderer<T> {
    ...
    private static void renderAt(AmbersolBlockEntity ambersol, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn) {
        ...
        if (scale > 0.0F) {
            ...
            for (int i = 0; i < lights; i++) {
                ...
                shineOriginVertex(lightConsumer, matrix4f, posestack$pose, j, u, v);
                shineLeftCornerVertex(lightConsumer, matrix4f, posestack$pose, length, width, u, v);
                shineRightCornerVertex(lightConsumer, matrix4f, posestack$pose, length, width, u, v);
                shineLeftCornerVertex(lightConsumer, matrix4f, posestack$pose, length, width, u, v);
    ...
    private static void shineOriginVertex(VertexConsumer p_114220_, Matrix4f p_114221_, PoseStack.Pose pose, int p_114222_, float xOffset, float yOffset) {
        p_114220_.addVertex(p_114221_, 0.0F, 0.0F, 0.0F).setColor(SHINE_CENTER_R, SHINE_CENTER_G, SHINE_CENTER_B, 230).setUv(xOffset + 0.5F, yOffset).setOverlay(NO_OVERLAY).setUv2(240 & 0xFFFF, 240 >> 16).setNormal(pose, 0.0F, 1.0F, 0.0F);
    }
    ...
    private static void shineLeftCornerVertex(VertexConsumer p_114215_, Matrix4f p_114216_, PoseStack.Pose pose, float p_114217_, float p_114218_, float xOffset, float yOffset) {
        p_114215_.addVertex(p_114216_, -HALF_SQRT_3 * p_114218_, p_114217_, 0).setColor(SHINE_R, SHINE_G, SHINE_B, 0).setUv(xOffset, yOffset + 1).setOverlay(NO_OVERLAY).setUv2(240 & 0xFFFF, 240 >> 16).setNormal(pose, 0.0F, -1.0F, 0.0F);
    }
    ...
    private static void shineRightCornerVertex(VertexConsumer p_114224_, Matrix4f p_114225_, PoseStack.Pose pose, float p_114226_, float p_114227_, float xOffset, float yOffset) {
        p_114224_.addVertex(p_114225_, HALF_SQRT_3 * p_114227_, p_114226_, 0).setColor(SHINE_R, SHINE_G, SHINE_B, 0).setUv(xOffset + 1, yOffset + 1).setOverlay(NO_OVERLAY).setUv2(240 & 0xFFFF, 240 >> 16).setNormal(pose, 0.0F, -1.0F, 0.0F);
    }
