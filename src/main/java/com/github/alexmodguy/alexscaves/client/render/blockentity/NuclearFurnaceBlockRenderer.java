...
public class NuclearFurnaceBlockRenderer<T extends NuclearFurnaceBlockEntity> implements BlockEntityRenderer<T> {
    ...
    public void render(T furnace, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn) {
        ...
        MODEL.renderToBuffer(poseStack, bufferIn.getBuffer(getRenderTypeFor(furnace, false)), combinedLightIn, combinedOverlayIn, 0xFFFFFFFF);
        MODEL.renderToBuffer(poseStack, bufferIn.getBuffer(getRenderTypeFor(furnace, true)), 240, combinedOverlayIn, 0xFFFFFFFF);
        poseStack.popPose();
    }
