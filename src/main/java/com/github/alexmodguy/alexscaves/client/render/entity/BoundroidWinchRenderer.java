...
public class BoundroidWinchRenderer extends MobRenderer<BoundroidWinchEntity, BoundroidWinchModel> {
    ...
    public static void renderChain(Vec3 to, PoseStack poseStack, VertexConsumer buffer, int packedLightIn, int overlayCoords) {
        ...
        //x links
        buffer.addVertex(matrix4f, chainOffset, 0, 0).setColor(255, 255, 255, 255).setUv((float) 0, (float) chainLength).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(posestack$pose, 0.0F, -1.0F, 0.0F);
        buffer.addVertex(matrix4f, chainWidth + chainOffset, 0, 0).setColor(255, 255, 255, 255).setUv((float) chainWidth, (float) chainLength).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(posestack$pose, 0.0F, -1.0F, 0.0F);
        buffer.addVertex(matrix4f, chainWidth + chainOffset, chainLength, 0).setColor(255, 255, 255, 255).setUv((float) chainWidth, (float) 0).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(posestack$pose, 0.0F, -1.0F, 0.0F);
        buffer.addVertex(matrix4f, chainOffset, chainLength, 0).setColor(255, 255, 255, 255).setUv((float) 0, (float) 0).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(posestack$pose, 0.0F, -1.0F, 0.0F);
        float pixelSkip = 2.5F / 16F;
        //z links
        buffer.addVertex(matrix4f, 0, pixelSkip, chainOffset).setColor(255, 255, 255, 255).setUv((float) chainWidth, (float) chainLength + pixelSkip).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(posestack$pose, 0.0F, -1.0F, 0.0F);
        buffer.addVertex(matrix4f, 0, pixelSkip, chainWidth + chainOffset).setColor(255, 255, 255, 255).setUv((float) chainWidth * 2, (float) chainLength + pixelSkip).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(posestack$pose, 0.0F, -1.0F, 0.0F);
        buffer.addVertex(matrix4f, 0, chainLength + pixelSkip, chainWidth + chainOffset).setColor(255, 255, 255, 255).setUv((float) chainWidth * 2, (float) pixelSkip).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(posestack$pose, 0.0F, -1.0F, 0.0F);
        buffer.addVertex(matrix4f, 0, chainLength + pixelSkip, chainOffset).setColor(255, 255, 255, 255).setUv((float) chainWidth, (float) pixelSkip).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(posestack$pose, 0.0F, -1.0F, 0.0F);
        poseStack.popPose();
    }
    ...
    class LayerGlow extends RenderLayer<BoundroidWinchEntity, BoundroidWinchModel> {
        ...
        public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, BoundroidWinchEntity entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
            ...
            int argb = ((int)(alpha * 255) << 24) | ((int)(1.0F * 255) << 16) | ((int)(1.0F * 255) << 8) | (int)(1.0F * 255);
            this.getParentModel().renderToBuffer(matrixStackIn, ivertexbuilder, packedLightIn, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), argb);
        }
    }
}
