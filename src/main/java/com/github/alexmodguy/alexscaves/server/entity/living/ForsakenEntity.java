    ...
    protected void customServerAiStep() {
        ...
        if(this.isInWall()){
            if (this.destroyBlocksTick > 0) {
                ...
                if (this.destroyBlocksTick == 0 && this.level().getGameRules().getBoolean(net.minecraft.world.level.GameRules.RULE_MOBGRIEFING)) {
                    ...
                    for(int j = -1; j <= 1; ++j) {
                        for(int k2 = -1; k2 <= 1; ++k2) {
                            for(int k = 0; k <= 3; ++k) {
                                ...
                                if (blockstate.canEntityDestroy(this.level(), blockpos, this) && !blockstate.is(ACTagRegistry.UNMOVEABLE)) {
