...
public class VallumraptorEntity extends DinosaurEntity implements IAnimatedEntity, ICustomCollisions, PackAnimal, ChestThief, TargetsDroppedItems {
    ...
    public boolean canTargetItem(ItemStack stack) {
        if (stack.is(ACBlockRegistry.VALLUMRAPTOR_EGG.get().asItem())) {
            return false;
        }
        if (stack.is(ACTagRegistry.VALLUMRAPTOR_STEALS)) {
            return true;
        }
        // In 1.21, food properties are accessed via DataComponents
        net.minecraft.world.food.FoodProperties foodProps = stack.get(net.minecraft.core.component.DataComponents.FOOD);
