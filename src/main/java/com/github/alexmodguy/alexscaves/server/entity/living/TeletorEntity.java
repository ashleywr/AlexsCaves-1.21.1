...
public class TeletorEntity extends Monster {
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficultyIn, MobSpawnType reason, @javax.annotation.Nullable SpawnGroupData spawnDataIn) {
        ...
        if (level().getRandom().nextFloat() < 0.25F * (f + 0.5F)) {
            int enchantLevel = (int) (5.0F + f * (float) level().getRandom().nextInt(18));
            stack = net.minecraft.world.item.enchantment.EnchantmentHelper.enchantItem(level().getRandom(), stack, enchantLevel, level().registryAccess(), Optional.empty());
        }
    ...
    public ItemStack createItemStack(RandomSource random) {
        if (HELD_ITEM_POSSIBILITIES == null || HELD_ITEM_POSSIBILITIES.isEmpty()) {
            HELD_ITEM_POSSIBILITIES = net.minecraft.core.registries.BuiltInRegistries.ITEM.stream().filter(item -> item.builtInRegistryHolder().is(ACTagRegistry.TELETOR_SPAWNS_WITH)).collect(ImmutableList.toImmutableList());
        }
    ...
    protected void dropEquipment() {
        ...
        if (weapon instanceof MagneticWeaponEntity magneticWeapon) {
            ...
            boolean hasVanishing = net.minecraft.world.item.enchantment.EnchantmentHelper.has(itemstack, net.minecraft.world.item.enchantment.EnchantmentEffectComponents.PREVENT_EQUIPMENT_DROP);
            if (!itemstack.isEmpty() && !hasVanishing && this.random.nextFloat() < f) {
    ...
    public boolean canBeAffected(MobEffectInstance effectInstance) {
        return super.canBeAffected(effectInstance) && !effectInstance.is(ACEffectRegistry.MAGNETIZING);
    }
