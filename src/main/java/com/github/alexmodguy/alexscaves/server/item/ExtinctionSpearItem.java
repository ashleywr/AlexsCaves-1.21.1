...
public class ExtinctionSpearItem extends SpearItem implements DemonicRarity {
    ...
    public void onUseTick(Level level, LivingEntity living, ItemStack stack, int timeUsing) {
        if(timeUsing == getUseDuration(stack, living)){
            level.playSound((Player) null, living, ACSoundRegistry.EXTINCTION_SPEAR_SUMMON.get(), SoundSource.PLAYERS, 1.0F, 1.0F);
            int grottoHeads = 3 + stack.getEnchantmentLevel(living.registryAccess().lookupOrThrow(net.minecraft.core.registries.Registries.ENCHANTMENT).getOrThrow(ACEnchantmentRegistry.HERD_PHALANX));
    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity hurtEntity, LivingEntity player) {
        hurtEntity.setRemainingFireTicks(100);
        ...
        dinosaurSpirit.setEnchantmentLevel(stack.getEnchantmentLevel(player.registryAccess().lookupOrThrow(net.minecraft.core.registries.Registries.ENCHANTMENT).getOrThrow(ACEnchantmentRegistry.CHOMPING_SPIRIT)));
