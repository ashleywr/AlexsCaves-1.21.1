...
public class CandyCaneHookEntity extends ThrowableProjectile {
    ...
    public CandyCaneHookEntity(Player player, Level level, ItemStack itemstack, boolean offhand) {
        ...
        double launchDist = 0.5D + EnchantmentHelper.getItemEnchantmentLevel(level.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(ACEnchantmentRegistry.FAR_FLUNG), itemstack) * 0.2D;
        ...
        this.setDamage(EnchantmentHelper.getItemEnchantmentLevel(level.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(ACEnchantmentRegistry.SHARP_CANE), itemstack) * 3.0F);
        if(EnchantmentHelper.getItemEnchantmentLevel(level.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(ACEnchantmentRegistry.STRAIGHT_HOOK), itemstack) > 0){
    ...
    protected double getDefaultGravity() {
        return isReeling() || this.entityData.get(RESISTS_GRAVITY) && this.getDeltaMovement().horizontalDistance() > 0.05F ? 0 : 0.08D;
    }
